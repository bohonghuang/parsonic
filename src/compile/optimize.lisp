                                      ...
                                      :else
                                        :unless (loop :with unused := t
                                                      ...
                                                      :when (and (eq var env-var) (not (eq val env-val)))
                                                        :do (setf unused nil))
                                          :collect arg)))
           (if (equal bindings (mapcar (rcurry #'subseq 0 2) args))
           ...
           ((lambda lambda-list &rest body)
            (multiple-value-bind (declarations body) (body-declarations body)
              (if (and (equal body `((with-codegen ,(ignore-errors (second (first body))))))
                       ...
                       (null (intersection lambda-list lambda-list-keywords)))
                  `(parser/let ,(mapcar #'list lambda-list (mapcar #'second parsers)) ,(flatmap->let (second (first body))))
  ...
  (if (consp form)
      (destructuring-case form
        ...
        ((parser/list &rest parsers)
         (let ((eql-count (loop :for count := 0 :then (if (eq op 'parser/eql) (1+ count) 0)
                                :for (op) :in parsers
                                :finally (return count))))
           (if (>= eql-count *eql-list->eql*-threshold*)
               (loop :for body := `(parser/eql* ,(mapcar #'second (last parsers eql-count))) :then `(parser/cons ,(eql-list->eql* arg) ,body)
                     :for arg :in (nreverse (butlast parsers eql-count))
                     :finally (return body))
