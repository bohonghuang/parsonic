...
(defun codegen (form)
  (flet ((ignore-results-p ()
    ...
    (destructuring-ecase form
      ...
      ((parser/eql* object)
       (let* ((type (cond
                      ((every #'characterp object) '(simple-array character (*)))
                      ((every (rcurry #'typep '(mod 256)) object) '(simple-array (unsigned-byte 8) (*)))
                      (t 'simple-vector)))
              (name (intern (format nil "~A-~A" 'parser/eql* type) #.*package*))
              (object (coerce object type)))
         (with-gensyms (input expected result position)
           (funcall *codegen-labels*
                    (let ((*codegen-blocks* (cons name *codegen-blocks*)))
                      (with-fresh-stack
                        (list `(,name (,input)
                                      (loop :for ,expected :across (the ,type ,input)
                    ...
                    (codegen `(parser/call ,name ,object))))))
