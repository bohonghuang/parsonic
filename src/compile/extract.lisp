  ...
  (typecase tree
    ((cons symbol list)
     (destructuring-case tree
       ...
       ((t &rest args)
        (declare (ignore args))
        (+ (if (parser-symbol-name (car tree)) 1 0)
...
(defun extract/compile (form)
  (flet ((next-units (units)
           (loop :for unit :in (sort units #'> :key #'parser-unit-cost)
                 ...
                 :when (and (>= count *parser-unit-extract-threshold-count*)
                   ...
                   :collect unit)))
    (loop :for (unit . units)
            := (let ((*parser-units* (make-hash-table :test #'equal)))
                 (setf form (parser-unit-walk (lexical->unit-args (remove-intermediate-units form))))
                 (next-units (hash-table-values *parser-units*)))
              :then (next-units units)
          :while unit
