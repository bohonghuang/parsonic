  ...
  (destructuring-ecase form
    ...
    ((parser/cons car cdr)
     (trie-extract-prefix-merge-branches
      (loop :for (key . op-car) :in (trie-ensure-success (trie-extract-prefix car))
            :if key
    ...
    ((parser/rep parser from to)
     (if (integerp from)
         (trie-extract-prefix-merge-branches
          (nconc
           (loop :for (key . op) :in (trie-ensure-success (trie-extract-prefix parser))
                 :do (assert key)
    ...
    ((parser/unit signature body)
     (destructuring-bind (name lambda-list) signature
       (loop :for (key . op) :in (trie-ensure-success (trie-extract-prefix body))
