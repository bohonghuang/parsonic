...
(declaim (ftype (function (cons) (values non-negative-integer non-negative-integer)) compute-bounds))

(defun compute-bounds-1/eval (form)
  (destructuring-ecase form
    ...
    ((parser/cut parser)
     (compute-bounds parser))))

(defvar *compute-bounds*)

(defun compute-bounds/eval (form)
  (let ((*compute-bounds* #'compute-bounds-1/eval))
    (compute-bounds form)))
